...
type Tab = "favorites" | "history" | "orders";
...
export default function AccountPage() {
  ...
  const loadTab = useCallback(
    async (opts?: { force?: boolean }) => {
      if (!user) return;
      const key = `${tab}|${tab === "favorites" ? favGroup : ""}`;
      const cached = cacheRef.current.get(key);
      const hasCached =
        (tab === "favorites" && cached?.favorites) ||
        (tab === "history" && cached?.history) ||
        (tab === "orders" && cached?.orders);

      if (hasCached && !opts?.force) {
        if (tab === "favorites") {
          setFavorites(cached!.favorites || []);
          if (cached!.favGroups) setFavGroups(cached!.favGroups);
        } else if (tab === "history") setHistory(cached!.history || []);
    ...
    <div className="pb-16 md:pb-24">
      ...
      >
        <div className="mx-auto max-w-[800px] px-4 py-8 md:px-6 md:py-12">
          ...
          <div className="flex items-start gap-4 md:gap-5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <h1 className="text-h3 font-bold text-ink md:text-h2">{nickname || t("account.title")}</h1>
                {user.isVip && (
          ...
          {/* Messages: mobile → /notifications page; PC → modal */}
          <Link
            href="/notifications"
            className="mt-6 flex items-center gap-3 rounded-2xl border border-line/80 bg-surface/80 px-4 py-3.5 transition-colors hover:bg-surface-2 md:hidden"
          >
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-brand/15">
              <Bell className="h-4 w-4 text-brand" />
            </span>
            <span className="min-w-0 flex-1 text-body-sm font-medium text-ink">{t("account.messages")}</span>
            <ChevronRight className="h-4 w-4 shrink-0 text-ink-subtle" />
          </Link>
          <button
            type="button"
            onClick={() => setNotifOpen(true)}
            className="mt-8 hidden w-full items-center gap-3 rounded-2xl border border-line/80 bg-surface/80 px-4 py-3.5 transition-colors hover:bg-surface-2 md:flex"
          >
          ...
          <div className="mt-6 overflow-hidden rounded-2xl border border-line/80 bg-surface">
            ...
            >
              ...
              <ChevronRight className={cn("h-4 w-4 text-ink-subtle transition-transform", showNickname && "rotate-90")} />
            </button>
            ...
            >
              ...
              <ChevronRight className={cn("h-4 w-4 text-ink-subtle transition-transform", showHelp && "rotate-90")} />
            </button>
            {showHelp && (
              <p className="border-t border-line px-4 py-3 text-body-sm text-ink-muted">{t("account.helpBody")}</p>
            )}
          </div>
        </div>
      </div>
