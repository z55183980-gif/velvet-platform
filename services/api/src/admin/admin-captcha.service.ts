...
export class AdminCaptchaService {
  ...
  isDisabled(): boolean {
    const raw = String(this.config.get<string>('AUTH_ADMIN_CAPTCHA_DISABLED') || '')
      .trim()
      .toLowerCase();
  ...
  issue(): CaptchaChallenge {
    ...
    const captchaId = `cap-${crypto.randomUUID().replace(/-/g, '')}`;
    const now = Date.now();
    this.prune(now);
  ...
  verify(captchaId: string, captchaCode: string): void {
    ...
    if (!id || !code) {
      throw new BizException(BizCode.BAD_REQUEST, '验证码不能为空');
    }
